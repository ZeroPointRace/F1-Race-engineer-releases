...
| Network | **Continuous internet connection recommended** — required for license checks **and for the voice engineer** (the voice is generated by an online service). Without internet the app still runs, but there is **no voice**. If there is no connection at launch (or a firewall blocks it), a warning appears in the main menu — check your connection/firewall. |
